...
def test_complete_mining_workflow():
    ...
    economic_filter = Filter(name="is_economic", children=["[total_value] >= [cutoff_grade]"])
    
    cs = CalcSet([
        block_au, block_ag, block_cu, block_tonnage,
        au_price, ag_price, cu_price,
        au_value, ag_value, cu_value,
        total_value, cutoff, economic_filter
    ])
...
def test_dataframe_workflow_with_filtering():
    ...
    conditional = Number(name="weighted_sum", children=[If(rows=[ifrow], otherwise=["[sum]"])])
...
def test_query_and_rename_workflow():
    ...
    cs_prefixed = cs.rename(
        items={r"^output_": "final_"},
        variables={r"^input_": "raw_"},
        regex=True
    )
...
def test_complex_conditional_workflow():
    ...
    a_cond = IfRow(
        condition=["([score] >= 90) and ([attendance] >= 90)"],
        value=["'A'"]
    )
    b_cond = IfRow(
        condition=["([score] >= 80) and ([attendance] >= 80)"],
        value=["'B'"]
    )
    c_cond = IfRow(
        condition=["([score] >= 70) and ([attendance] >= 70)"],
        value=["'C'"]
    )
